...
for p in product((True, False), repeat=len(variables)):
    ...
    if len(variables) > 1:
        ...
        for x in range(len(operators)):
            if operators[x] == '/\\':
                truth = t._and(truth, storage[x+1])#truth and storage[x+1]
            elif operators[x] == '\/':
                truth = t._or(truth, storage[x+1])#truth or storage[x+1]
            elif operators[x] == '>':
                truth = t._imply(truth, storage[x+1])#not truth or storage[x+1]
